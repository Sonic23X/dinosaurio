	...
	$( "#editar_password" ).submit(function( event ) {
		...
		$.ajax({
			 type: "POST",
			 url: "http://localhost/dinosaurio/User/Pass",
			 data: "id=" + $('#user_id_mod'). val() +
			 "&pass=" + $('#user_password_new3').val() +
			 "&rpass=" + $('#user_password_repeat3').val(),
				beforeSend: function(objeto){
				 $("#resultados_ajax3").html("Mensaje: Cargando...");
				 },
			 success: function(datos){
			 $("#resultados_ajax3").html(datos);
			 $('#actualizar_datos3').attr("style", "display: none");
			 }
	 		});
